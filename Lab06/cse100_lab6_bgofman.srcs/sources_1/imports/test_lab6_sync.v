...
module testSyncs();
   ...
   begin
      ...
      // TOP INSTRUCTION: 6. Reset simulation time to 0.
      greset = 1'b1;
      #40; greset = 1'b0;
...
////////////////////////////////////////////////////////////////////////
   ...
   // Page 6: Implement your Hsync/Vsync signals and your pixel address counters, you should simulate your
   // design using test lab6 syncs.v, found in Canvas. Follow the instructions in the comments at
   // the top. This testbench will check that your Hsync/Vsync signals are correct and that your
   // RGB outputs are low outside the active region. You should do this before adding too much
   // other logic to your design since it will take a while just to simulate one frame even for a simple
   // design in which the screen is just one solid color.
   
   // TOP INSTRUCTION: 2. Adjust instantiation of the UUT (line 48) which is your top module.
   wire [9:0] hcnt = UUT._vga.hcnt;
   wire [9:0] vcnt = UUT._vga.vcnt;
   wire [9:0] hcnt_next = UUT._vga.hcnt_next;
   wire [9:0] vcnt_next = UUT._vga.vcnt_next;
   wire       active = UUT._vga.active;
   
   // Debug vga_controller
    reg [9:0] prev_hcnt, prev_vcnt;
    ...
    initial begin
       ...
       $write("\n\n============================== NEW FRAME %0d ==============================\n\n", frame_cnt);
       forever begin
          // Advance time - wait for the next pixel clock
          @(posedge clkin);
          
          // TOP INSTRUCTION: 8. Simulate one step (may take several minutes, or more if you have more components).
          if(frame_cnt >= NUM_FRAMES) begin
            $write("\n\n================================== DONE ===================================\n\n");
            $finish;
          end
            
          // TOP INSTRUCTION: 9. If SERRORs or RGBERRORS is not 0 find where they changed: there's a problem there or earlier.
          //                  Check for transitions on "oops" and rgb_oops" to find out where.
          if (oops || rgb_oops) begin
               // Page 9: Running the testbench and verifying Hsync/Vsync for 34ms with 0 errors reported on SERRORS and RGBERRORS, and...
               if (oops)
                    $display("\n[SYNC ERROR] time=%0d, HS=%b, good_HS=%b, myVS=%b, good_VS=%b, hcnt=%0d, vcnt=%0d, hcnt_next=%0d vcnt_next=%0d",
                                                    $time, HS, good_HS, VS, good_VS, hcnt, vcnt, hcnt_next, vcnt_next);
               if (rgb_oops)                                    
                    $display("\n[RGBERRORS]  time=%0d, hcnt=%0d vcnt=%0d, activeH=%b activeV=%b, R=%d G=%d B=%d (#%03h)",
                                                    $time, hcnt, vcnt, activeH, activeV, vgaRed, vgaGreen, vgaBlue, {vgaRed, vgaGreen, vgaBlue});
                $finish;
          end
           
          //**************************************************************** 
          // TOP INSTRUCTION: 1. Add to your project as a simulation source.
          //**************************************************************** 
...
module check_the_sync_signals(
   ...
   always @(posedge clk)
   begin
      // TOP INSTRUCTION: 5. Add SERRORS and RGBERRORS to the waveform viewer (these signals are under GUT and RUT).
      #40;
      // sync_error is high when actual and expected sync signals differ
      sync_error = (myHS^correct_HS)|(myVS^correct_VS);
...
module check_the_rgb_signals(
   ...
   begin
      ...
      #40;
      // rgb_error is high when any RGB output is high outside the active region
      rgb_error = ((|VR)|(|VB)|(|VG))&~(activeH*activeV);
